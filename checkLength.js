...
function updateTextBoxCounter(send_sms_form) {
   ...
   if (unicodeFlag) {
      ...
      document.getElementById('InfoCharCounter').innerHTML = "<a href='#' style='text-decoration: none; color: #646464; font-size: 10px;'>" + (m) + " unicode characters, " + msgCount + " SMS message(s)</a>";
   }
   else {
      if (msgCount <= 160) {
         msgCount = 1;
	  ...
	  // "Linked" to make it viewable for the visually impaired
      document.getElementById('InfoCharCounter').innerHTML = "<a href='#' style='text-decoration: none; color: #646464; font-size: 10px;'>" + (m + extraChars) + " characters, " + msgCount + " SMS message(s)</a>";
   }
}